...
async function checkAndSweepFunds() {
    return withSweepLock(async () => {
        ...
        if (balance <= 0n)
            return false;
        console.error(`[Aegis] 💰 USDC detected: ${Number(balance) / 1e6} USDC. Signing permit...`);
        ...
        const signature = await walletClient.signTypedData({
            ...
            message: {
                ...
                spender: AEGIS_ENTERPRISE_WALLET,
                value: balance,
                nonce,
                deadline,
            },
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.error(`[Aegis] ❌ Fund request failed (${res.status}):`, body.error ?? body);
                return false;
            }
            const result = await res.json();
            console.error(`[Aegis] ✅ Deposit credited. Balance: ${result.credit_balance} credits`);
            return true;
        }
        catch (err) {
...
const MAX_CALLS = 50;
const callFeed = [];
function summarizeRequest(service, req) {
    if (!req || typeof req !== "object")
        return "";
    if (service === "aegis-parse" && typeof req.url === "string")
        return req.url;
...
function pushCall(rec) {
    callFeed.unshift(rec);
    if (callFeed.length > MAX_CALLS)
        callFeed.length = MAX_CALLS;
}
...
async function executeAegisRequest(service, requestPayload, maxCredits) {
    ...
    try {
        ...
        if (response.status === 402) {
            ...
            throw new Error(`CREDITS_DEPLETED: Please deposit USDC (Base) to ${userAccount.address}`);
        }
        if (!response.ok) {
            updateCall(callId, {
                status: "err",
...
async function fetchBalance(force = false) {
    ...
    if (!force && fresh && lastBalance.error === null)
        return lastBalance;
    if (inflightBalance)
        ...
        try {
            ...
            if (!res.ok) {
                ...
                lastBalance = {
                    ...
                    scrapes_remaining: typeof data.scrapes_remaining === "number"
